...
{
	m_animSprite = AddComponent<AnimationSprite>(RS__BARRY_PNG, 8, 3, false);
	...
	m_animSprite->AddAnimation(Animation(0, 7, 0.3f), "RIGHT");
	m_animSprite->AddAnimation(Animation(8, 13, 0.3f), "DOWN");
	m_animSprite->AddAnimation(Animation(14, 15, 0.3f), "IDLE");
	m_animSprite->AddAnimation(Animation(16, 21, 0.3f), "UP");
...
void Barry::update()
{
	...
	else if (direction.y * direction.y > 0.1f) m_animSprite->SetCurrentAnimation(direction.y < 0.f ? "UP" : "DOWN");
